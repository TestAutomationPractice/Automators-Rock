...
public class AudioOnlyShow extends Base {
	...
	@Test(description = "Create an Audio only Show", enabled = true)
	...
	public void createshow() throws Throwable, Exception {
		...
		showdetails.selectMediaType(ShowType.Audio_Only.getShowType());
		...
		Media mediaAudio = PageFactory.initElements(driver, Media.class);
		mediaAudio.setDependencies(logger, testDataHelper);
		mediaAudio.clickUploadMedia(testDataDto,MediaType.Audio,"p.mp3");
		mediaAudio.enterSpeakerName("Speaker");
		mediaAudio.enterSpeakerCompany("Nagarro");
		mediaAudio.enterSpeakerTitle("Tester");
		mediaAudio.enterSequence("1");
		mediaAudio.clickAdd();
		mediaAudio.clickContinue();
